...
<head>
  ...
  <title>Reset Password - Tabassum Fashion</title>
  ...
  <div class="min-h-screen auth-bg flex items-center justify-center py-12 px-4">
    <div class="w-full max-w-md">
      ...
      <div class="bg-white rounded-3xl p-8 shadow-2xl">
        <h1 class="text-2xl font-extrabold text-center">Reset Password</h1>
        <p class="text-gray-500 text-center mt-2">Choose a new password for your account</p>
        ...
        <form method="POST" action="{{ route('password.store') }}" class="mt-8 space-y-5">
          ...
          <div>
            ...
            <input type="email" id="email" class="w-full border-2 @error('email') border-red-400 @else border-gray-200 @enderror rounded-xl px-4 py-3.5 focus:border-primary-600 focus:outline-none" type="email" name="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username" placeholder="your@email.com" readonly>
            @error('email')
          ...
          <div>
            ...
            <div class="relative">
              <input type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password"
                class="w-full border-2 @error('password_confirmation') border-red-400 @else border-gray-200 @enderror rounded-xl px-4 py-3.5 focus:border-primary-600 focus:outline-none pr-12"
                placeholder="Re-enter new password">
              <button type="button" onclick="togglePassword('password_confirmation', this)" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><i class="fa-regular fa-eye"></i></button>
            </div>
          ...
          <!-- Submit -->
          <button type="submit" class="w-full bg-primary-600 hover:bg-primary-700 text-white py-3.5 rounded-full font-bold transition flex items-center justify-center gap-2 shadow-lg shadow-primary-600/25">
            <i class="fa-solid fa-check"></i> Reset Password
          </button>
        </form>

        <p class="text-center mt-6"><a href="{{ route('login') }}" class="text-gray-400 hover:text-gray-600 text-sm"><i class="fa-solid fa-arrow-left mr-1"></i> Back to Login</a></p>
      </div>
    </div>
  </div>
